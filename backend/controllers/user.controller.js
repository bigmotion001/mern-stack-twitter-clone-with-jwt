...
export const UsersWeAreFollowing = async (req, res) => {
  const userId = req.userId;
  try {
    ...
    if (!following) return res.status(401).json({ message: "user nof found" });
...
export const SendMessage = async (req, res) => {
  try {
    ...
    if (receiverSocketId) {
      const ii = io.to(receiverSocketId).emit("newMessage", newMessage);
    }
...
export const GetMessage = async (req, res) => {
  try {
    ...
    // const receiver = await User.findById(userToChatId).select("-password");
